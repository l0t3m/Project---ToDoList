...
def search_task(query):
    ...
    return db.get_dicts(f"SELECT * FROM tasks WHERE category LIKE '{query}%' or description like '{query}%' or date like '{query}%';")
...
def get_category(category_id):
    ...
    return list(db.get_dicts(f"SELECT * FROM categories WHERE category_id = '{category_id}'"))
...
def search_category(category_name):
    ...
    return db.get_dicts(f"SELECT * FROM categories WHERE category_name LIKE '{category_name}%'")
