...
def setup(filename="tasks.sqlite"):
    '''Creating the DB if not exists and inserting basic data.'''
...
def setup_basic_info(filename="tasks.sqlite"):
    ...
    if os.path.exists(filename) == True:
        with sqlite3.connect(filename) as conn:
            ...
            cur.execute("INSERT INTO tabs (tab_name, tab_url) VALUES ('All Tasks', '/'),('Add a Task','/add-task'),('All Categories','/categories'),('Add a Category','/add-category');")
            conn.commit()
...
def get_dicts(sql="SELECT * FROM tasks", filename="tasks.sqlite"):
    '''Gets a sql and returns a dict containing the title(key) and row(value).'''
    if os.path.exists(filename) == True:
        tasks = query_db(sql, filename)["rows"]
        keys = query_db(sql, filename)["keys"]
